-- sesion3.sql: Script para la Sesión 3
...
DECLARE
    ...
    v_pedido_id NUMBER := 103; -- ID del pedido a evaluar
...
BEGIN
    ...
    -- Clasificación mediante estructura IF-ELSIF-ELSE 
    IF v_total_venta > 1000 THEN
        v_categoria := 'Alto';
    ELSIF v_total_venta > 500 THEN
        v_categoria := 'Medio';
    ELSE
        v_categoria := 'Bajo';
    END IF;
    ...
    DBMS_OUTPUT.PUT_LINE('Monto Total: ' || v_total_venta);
    DBMS_OUTPUT.PUT_LINE('Clasificación: ' || v_categoria);

EXCEPTION
    -- Manejo de errores si el pedido no existe 
    WHEN NO_DATA_FOUND THEN
        DBMS_OUTPUT.PUT_LINE('Error: El pedido ' || v_pedido_id || ' no existe.');
    WHEN OTHERS THEN
